...
output_path5 <- getOptionValue("--output_path5")
if (!dir.exists(output_path5)) {
  dir.create(output_path5, recursive = TRUE)
}
absolute_path5 <- normalizePath(output_path5)
...
for (i in seq_along(file_list)) {
  ...
  file_name <- file_list_name[i]
  file_name_read <- readLines(file_name, n = 1)
  name_file <- strsplit(file_name_read, "_vs_")[[1]]

# Récupérer les parties
  ref_level <- name_file[2]  # Ce qui est après "_vs_"
  mutant_level <- name_file[1]  # Ce qui est avant"_vs_"

  # Génération du nom du fichier de sortie
  output_file1 <- paste0("diffexp_", file_name_read, ".html")
